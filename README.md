...
- **Web アプリは LLM を呼ばない**。OCR とチャット回答は skill を発動した Claude Code
...
- エージェントのポーリングは**バックグラウンド実行** (Bash run_in_background)。待機中も
  ...
  エージェントが誰もポーリングしていないときは UI に警告バナーが出る
...
- 右下の「データについて質問」— 保存データについてエージェントと Q&A
